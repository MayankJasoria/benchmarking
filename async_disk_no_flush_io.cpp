...
array<long, 3> perform_write(int fd, string &data_to_write) {
    ...
    auto fsync_duration = chrono::duration_cast<chrono::nanoseconds>(after_fsync_time - start_time).count();
    spdlog::debug("Total time taken until aio_fsync returns: {} nanoseconds", fsync_duration);
    ...
    if (aio_error(&cb) != 0) {
        ...
        if (bytes_written == write_size) {
            ...
            return {initiation_duration, duration_before_fsync, fsync_duration};
        }
...
void writeResultsToFile(const vector<array<long, 3>>& times, int msg_size) {
    ...
    if (outputFile.is_open()) {
       ...
       outputFile << "initiation_duration_nsec\twrite_duration_nsec\taio_fsync_duration_nsec\n";
