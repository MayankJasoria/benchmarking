...
array<long, 4> perform_write(int fd, string &data_to_write) {
	...
	// 6. Force write to disk (aio_fsync)
	auto fsync_start_time = chrono::high_resolution_clock::now();
	if (aio_fsync(O_SYNC, &cb) < 0) {
		spdlog::error("Error flushing file (aio_fsync): {}", strerror(errno));
		close(fd);
		exit(EXIT_FAILURE);
	}
	auto fsync_end_time = chrono::high_resolution_clock::now();
	auto fsync_duration = chrono::duration_cast<chrono::nanoseconds>(fsync_end_time - start_time).count();
	spdlog::debug("Time taken for aio_fsync: {} nanoseconds", fsync_duration);

	// 7. Get the result of the fsync operation
	auto result_start_time = chrono::high_resolution_clock::now();
	ssize_t fsync_result = aio_return(&cb);
	if (fsync_result == -1) {
		spdlog::error("Error getting result of aio_fsync: {}", strerror(errno));
		close(fd);
		exit(EXIT_FAILURE);
	}
	auto result_end_time = chrono::high_resolution_clock::now();
	auto result_duration = chrono::duration_cast<chrono::nanoseconds>(result_end_time - start_time).count();
	spdlog::debug("Time taken to get result of aio_fsync: {} nanoseconds", result_duration);

	// Check for write errors (we already checked the return of aio_write)
	if (aio_error(&cb) != 0) {
		spdlog::error("Asynchronous write error: {}", strerror(aio_error(&cb)));
		exit(EXIT_FAILURE);
	} else {
		ssize_t bytes_written = aio_return(&cb); // Get the result of the write again
		if (bytes_written == write_size) {
			spdlog::debug("Successfully wrote {} bytes", bytes_written);
			return {initiation_duration, duration_before_fsync, fsync_duration, result_duration};
		}
...
void writeResultsToFile(const vector<array<long, 4>>& times, int msg_size) {
	// Construct the output file name
	std::string filename = "/hdd2/rdma-libs/results/async_io_flush_" + std::to_string(msg_size) + ".txt";
	std::ofstream outputFile(filename);

	// Check if the file was opened successfully
	if (outputFile.is_open()) {
		// Write the header row
		outputFile << "initiation_duration_nsec\twrite_duration_nsec\tflush_duration_nsec\tflush_complete_nsec\n";

		// Write the data from the 'times' vector
		for (const auto& time_array : times) {
			outputFile << time_array[0] << "\t" << time_array[1] << "\t" << time_array[2] << "\t" << time_array[3] << "\n";
		}
...
int main(int argc, char* argv[]) {
	...
	string filename = "/hdd2/rdma-libs/files/aio_append_async_flush_test_" + to_string(num_bytes) + ".txt"; // Replace with your file path
	...
	vector<array<long, 4>> times(num_msgs);
	int message_count = 0;
	for (int i = 0, idx = 0; i < num_msgs; ++i, idx = (idx + 1) % saved_msgs_count) {
	string msg = saved_msgs[i];
		array<long, 4> durations = perform_write(fd, msg);
		times[i] = durations;
		message_count++;
	}
