...
#include "rlibv2/core/qps/mod.hh"
#include "rlibv2/core/utils/timer.hh"

using namespace rdmaio;
using namespace rdmaio::qp;
using namespace rdmaio::rmem;

DEFINE_string(addr, "192.168.252.211:8888", "Server address to connect to.");
...
int main(int argc, char **argv) {
	...
	if (cm.wait_ready(100000, 4) == IOCode::Timeout) // wait 1 second for server to ready, retry 2 times
		RDMA_LOG(4) << "connect to the " << FLAGS_addr << " timeout!";

	sleep(10);

	// 2. create the remote QP and connect
	auto qp_res = cm.cc_rc_msg("client_qp", FLAGS_cq_name, 4096, qp, FLAGS_use_nic_idx, QPConfig());
	...
	for (int i = 1; i <= FLAGS_msg_cnt; ++i) {
		...
		auto res_s = qp->send_normal(
		    {.op = IBV_WR_SEND_WITH_IMM, .flags = IBV_SEND_SIGNALED, .len = (u32)msg.size() + 1, .wr_id = 0},
	...
	RDMA_LOG(2) << "rc client send " << FLAGS_msg_cnt << " msg in " << passed_msec << " msec";
	return 0;
}